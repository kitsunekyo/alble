...
function nextGlobalDay(): number {
  const r = db.select({ m: max(sessions.global_day) }).from(sessions).get();
  return (r?.m ?? 0) + 1;
}

export function listSessions(): SessionDTO[] {
  const sessionRows = db.select().from(sessions).orderBy(asc(sessions.global_day)).all();
...
export function getSession(id: number): SessionDTO | null {
...
export function getTodaySession(): SessionDTO | null {
  const today = new Date().toISOString().slice(0, 10);
  const s = db.select().from(sessions).where(eq(sessions.date, today)).get();
  if (!s) return null;
  return getSession(s.id);
}

export function getOrCreateTodaySession(): SessionDTO {
...
export function updateSession(id: number, input: UpdateSessionInput): SessionDTO | null {
...
export function addStep(sessionId: number, input: CreateStepInput): StepDTO | null {
...
export function updateStep(stepId: number, input: UpdateStepInput): StepDTO | null {
  ...
  db.update(steps).set(patch).where(eq(steps.id, stepId)).run();
  const cur = db.select().from(steps).where(eq(steps.id, stepId)).get();
  return cur ? toStepDTO(cur) : null;
}

export function deleteStep(stepId: number): boolean {
  const step = db.select().from(steps).where(eq(steps.id, stepId)).get();
  if (!step) return false;
  db.delete(steps).where(eq(steps.id, stepId)).run();
  ...
  for (const s of remaining) {
    db.update(steps).set({ step_number: -s.step_number }).where(eq(steps.id, s.id)).run();
  }
  remaining.forEach((s, i) => {
    db.update(steps).set({ step_number: i + 1 }).where(eq(steps.id, s.id)).run();
  });
  return true;
}

export function countSessions(): number {
  const r = db.select({ c: sql<number>`count(*)` }).from(sessions).get();
  return r?.c ?? 0;
}
